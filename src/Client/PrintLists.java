        ...
        input.sort(new Comparator<Advertisement>() {
            public int compare(Advertisement o1, Advertisement o2) {
                LocalDateTime triggerTime =
                        LocalDateTime.ofInstant(Instant.ofEpochMilli(o1.getLastUpdate()),
                                TimeZone.getDefault().toZoneId());
                LocalDateTime triggerTime2 =
                        LocalDateTime.ofInstant(Instant.ofEpochMilli(o2.getLastUpdate()),
                                TimeZone.getDefault().toZoneId());
                if (triggerTime.compareTo(triggerTime2) > 0){
                    return -1;
                }
                if (triggerTime.compareTo(triggerTime2) < 0){
                    return 1;
                }
                return triggerTime.compareTo(triggerTime2);
            }
        });
        for (Advertisement a : input){
            System.out.println(a);
